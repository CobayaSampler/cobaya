...
class BBN(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        yhe = self.bbn.Y_He(
            self.provider.get_param("ombh2"), self.provider.get_param("nnu") - 3.044
        )
...
class BBN2(Theory):
    ...
    def calculate(self, state, want_derived=True, **params_values_dict):
        if want_derived:
            state["derived"] = {
                "YHe": self.bbn.Y_He(
                    params_values_dict["ombh2"], params_values_dict["nnu"] - 3.044
                )
            }
...
class TrivialNonLinearRatio(Theory):
    ...
    def get_non_linear_ratio(self, results):
        ...
        k_h = np.logspace(-4, np.log10(kh), 200)
        z = np.array(
...
def test_pk_binning(packages_path, skip_not_installed):
    ...
    _info: InputDict = {
        ...
        "likelihood": {"cmb": Pklike},
        "theory": {
            "camb": {"external_primordial_pk": True, "extra_args": {"WantTensors": True}},
            "my_pk": {
                "external": BinnedPk,
                "nbins": nbins,
                "k_min_bin": k_min_bin,
