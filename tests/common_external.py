...
info_method_unnamed_kwargs = {
    "half_ring": {
        "external": half_ring_instance.logp_unnamed_kwargs,
        "input_params": ["x", "y"],
    }
}
...
def body_of_test(info_logpdf, kind, tmpdir, derived=False, manual=False):
    ...
    info: InputDict = {
        ...
        "sampler": {
            "mcmc": {"max_samples": (10 if not manual else 5000), "learn_proposal": False}
        },
    }
    if derived:
        info["params"].update(
    ...
    if kind == "prior":
        ...
        assert np.allclose(
            ...
            np.sum(
                products["sample"][columns_priors[1:]].to_numpy(dtype=np.float64), axis=-1
            ),
