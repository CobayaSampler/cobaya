...
class _planck_2018_CamSpec_python(_DataSetLikelihood):
    ...
    def get_powerlaw_residuals(self, data_params):
        ...
        C_powerlaw = np.empty((4, lmax + 1))
        powerlaw_pivot=1500

        C_powerlaw[0,:] = self.llp1*(1.*ls/powerlaw_pivot)**Tilt[0]
        C_powerlaw[1,:] = self.llp1*(1.*ls/powerlaw_pivot)**Tilt[1]
        C_powerlaw[2,:] = self.llp1*(1.*ls/powerlaw_pivot)**Tilt[2]
        C_powerlaw[3,:] = self.llp1*(1.*ls/powerlaw_pivot)**Tilt[3]
