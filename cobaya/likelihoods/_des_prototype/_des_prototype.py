...
:Synopsis: DES likelihood. Well tested and agrees with likelihoods in DES chains for fixednu.
...
class _des_prototype(Likelihood):
    ...
    def get_theory(self, PKdelta, PK, bin_bias, shear_calibration_parameters,
        ...
        qgal = get_qgal()
        wq = get_wq()
        qs = 3 * omm * h2 * (1e5 / c) ** 2 * chis * (1 + self.zs) / 2 * wq
        ...
        def cl_kappas_Weyl(ls):
            cls = np.empty((self.nzbins, self.nzbins), dtype=np.object)
            cls[:, :] = None
            for f1, f2 in self.bin_pairs[0]:
                cls[f1, f2] = np.zeros(ls.shape)
            weight = np.ones(chis.shape)
            for ix, l in enumerate(ls):
                k = (l + 0.5) / chis
                weight[:] = 1
                weight[k < 1e-4] = 0
                weight[k >= PK.kmax] = 0
                tmp = dchis * weight * PK.P(self.zs, k, grid=False)
                for f1, f2 in self.bin_pairs[0]:
                    cls[f1, f2][ix] = np.dot(tmp, wq[f1] * wq[f2])
            return cls
        ...
        if self.use_hankel:
            ...
            if 'xip' in self.used_types or 'xim' in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index('xip')]:
                    cl = UnivariateSpline(ls_cl, np.dot(tmp, qs[f1] * qs[f2]), s=0)
            ...
            if 'xip' in self.used_types or 'xim' in self.used_types:
                for (f1, f2) in self.bin_pairs[self.data_types.index('xip')]:
                    cl = UnivariateSpline(
                        ls_cl, np.dot(tmp, qs[f1] * qs[f2]), s=0)(ls_bessel)
                    fac = ((1 + shear_calibration_parameters[f1]) *
    ...
    def logp(self, **params_values):
        ...
        PK = Pk_interpolators.get("weyl_weyl", None)
        wl_photoz_errors = [params_values.get(p, None) for p in
            ['DES_DzS1', 'DES_DzS2', 'DES_DzS3', 'DES_DzS4']]
        lens_photoz_errors = [params_values.get(p, None) for p in
            ['DES_DzL1', 'DES_DzL2', 'DES_DzL3', 'DES_DzL4', 'DES_DzL5']]
        bin_bias = [params_values.get(p, None) for p in
            ['DES_b1', 'DES_b2', 'DES_b3', 'DES_b4', 'DES_b5']]
        shear_calibration_parameters = [params_values.get(p, None) for p in
        ...
        theory = self.get_theory(
            ...
            intrinsic_alignment_A=params_values['DES_AIA'],
            intrinsic_alignment_alpha=params_values['DES_alphaIA'],
            intrinsic_alignment_z0=params_values['DES_z0IA'])
