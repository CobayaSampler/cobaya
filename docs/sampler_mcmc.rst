...
- **Dragging the fast parameters:** implies a number of intermediate steps when
  ...
  degeneracy betwen them. By default, if **on**, spends the same time doing dragging
  steps at it takes to compute a jump in the slow direction (make sure your likelihoods
  ``speed``'s are accurate; see below)
...
.. note::
   ...
   Oversampling is still WIP. Please, use dragging instead.
