...
Instead, if given as a matrix, you must also include the ``covmat_params`` property, listing the parameters in the matrix in the order in which they appear. Finally, ``covmat`` admits the special value ``auto`` that searchs for an appropriate covariance matrix in a database (see :doc:`Basic cosmology runs <cosmo_basic_runs>`).
...
.. note::
   ...
   A covariance matrix given via ``covmat`` does not need to contain the all the sampled parameters, and may contain additional ones unused in your run. For the missing parameters the specified input ``proposal`` (or refence, or prior) is used, assuming no correlations.
