...
from cobaya import likelihoods, samplers, theories
import types

# Inject fake "all" modules containing all internal classes, eg. cobaya.likelihoods.all

for classes, host in [(get_available_internal_classes(k, True), m) for k, m in
                      zip(kinds, [samplers, theories, likelihoods])]:
    host.all = types.ModuleType('all')
    for cls in classes:
        cls.__module__ = host.all.__name__
        setattr(host.all, cls.__name__, cls)
...
html_static_path = ['_static', 'theme_customisation']
