...
# Creating the list of external likelihoods -- it modifies the corresponding file
# on-the-fly and adds raw html, and deletes the modifications after processing it.
# WARNING: may generate some "git noise" depending on the system.

likelihood_external_list_file = "likelihood_external_list.rst"


def create_list_external(app, env, docnames):
    """Adds list of external likelihoods."""
    from cobaya_cosmo.generate_cosmo_likelihood_list import generate_external_list
    indent = 4 * " "
    with open(os.path.join(env.srcdir, likelihood_external_list_file), "a") as f:
        f.write("\n.. raw:: html\n\n")
        for line in generate_external_list().split("\n"):
            f.write(indent + line + "\n")


def cleanup_list_external(app, env):
    """Removes list of external likelihoods to revert to (hopefully) clean git version."""
    with open(os.path.join(env.srcdir, likelihood_external_list_file), "r") as f:
        lines = f.readlines()
    i_last = next(i for i, l in enumerate(lines) if "@@@@" in l)
    lines = lines[:i_last + 1]
    with open(os.path.join(env.srcdir, likelihood_external_list_file), "w") as f:
        f.writelines(lines)


def setup(app):
    app.connect('env-before-read-docs', create_list_external)
    app.connect('env-check-consistency', cleanup_list_external)
